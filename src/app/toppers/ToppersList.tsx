...
export function ToppersList({ pedidos }: Props) {
  ...
  const { pagaveisTodos, totalToppers, totalAReceber, totalAPagar, totalJaPago, contagens } =
    useMemo(() => {
      ...
      for (const p of pedidos) {
        ...
        // Etapa derivada, não os dois booleanos soltos: é o que impede um
        // topper já recebido, mas sem a solicitação marcada, de contar como
        // pendente.
        const etapa = etapaDoTopper(t);

        if (etapa !== "recebido") aReceber++;
        if (etapa === "pendente") pendentes++;
        if (etapa === "solicitado") solicitados++;
        if (etapa === "recebido") recebidos++;
  ...
  const filtros: { key: Filtro; label: string; count: number }[] = useMemo(
    () => [
      ...
      { key: "pendentes", label: "Falta pedir", count: contagens.pendentes },
      { key: "solicitados", label: "A caminho", count: contagens.solicitados },
